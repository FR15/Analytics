...
class TalkingDataAppAnalytics {
  static const MethodChannel _channel = const MethodChannel('TalkingData_AppAnalytics');
  ...
  static Future<void> onPageStart(String pageName) async {
    return await _channel.invokeMethod('onPageStart', <String, dynamic>{'pageName': pageName});
  }

  static Future<void> onPageEnd(String pageName) async {
    return await _channel.invokeMethod('onPageEnd', <String, dynamic>{'pageName': pageName});
  }

  // 自定义事件
  static Future<void> onEvent({@required String eventID, String eventLabel, Map params}) async {
    return await _channel
        .invokeMethod('onEvent', <String, dynamic>{'eventID': eventID, 'eventLabel': eventLabel, 'params': params});
  }

  static Future<void> onEventWithValue({@required String eventID, String eventLabel, Map params, double value}) async {
    return await _channel.invokeMethod('onEventWithValue',
        <String, dynamic>{'eventID': eventID, 'eventLabel': eventLabel, 'params': params, 'value': value});
  }

  static Future<void> setGlobalKV(String key, Object value) async {
    return await _channel.invokeMethod('setGlobalKV', <String, dynamic>{'key': key, 'value': value});
  }

  static Future<void> removeGlobalKV(String key) async {
    return await _channel.invokeMethod('removeGlobalKV', <String, dynamic>{'key': key});
  }

  // 统计用户注册
  // profileType - 第三方平台
  static Future<void> onRegister({String profileID, ProfileType profileType, String name}) async {
    return await _channel.invokeMethod('onRegister',
        <String, dynamic>{'profileID': profileID, 'profileType': profileType.toString().split('.')[1], 'name': name});
  }

  // 统计用户登录
  // profileType - 第三方平台
  static Future<void> onLogin({String profileID, ProfileType profileType, String name}) async {
    return await _channel.invokeMethod('onLogin',
        <String, dynamic>{'profileID': profileID, 'profileType': profileType.toString().split('.')[1], 'name': name});
  }

  static Future<void> onPlaceOrder({String orderId, int amount, String currencyType}) async {
    return await _channel.invokeMethod(
        'onPlaceOrder', <String, dynamic>{'orderId': orderId, 'amount': amount, 'currencyType': currencyType});
  }
  ...
  static Future<void> onCancelOrder({String orderId, int amount, String currencyType}) async {
    return await _channel.invokeMethod(
        'onCancelOrder', <String, dynamic>{'orderId': orderId, 'amount': amount, 'currencyType': currencyType});
  }

  static Future<void> onAddItemToShoppingCart(
      {String itemId, String category, String name, int unitPrice, int amount}) async {
  ...
  static Future<void> onViewItem({String itemId, String category, String name, int unitPrice}) async {
    return await _channel.invokeMethod(
        'onViewItem', <String, dynamic>{'itemID': itemId, 'category': category, 'name': name, 'unitPrice': unitPrice});
  }

  static Future<void> onViewShoppingCart(ShoppingCart shoppingCart) async {
    return await _channel.invokeMethod(
        'onViewShoppingCart', <String, dynamic>{'shoppingCartDetails': shoppingCart._shoppingCartDetails});
  }
}
